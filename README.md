...
처음에는 바로 상용화해도 될 만큼 완성도 높은 면접 시뮬레이터를 목표로 했지만, 구현할수록 부족한 점이 드러난 것이 가장 아쉬웠습니다. CC5로 사실적인 면접관을 제작했으나 Unity로 옮기며 품질이 낮아졌고, 면접 상황에 맞는 애니메이션 리소스가 부족해 다양한 표정과 제스처를 갖춘 능동적인 면접관을 온전히 구현하지 못했습니다. 백엔드 경험이 거의 없어 네트워크 통신에서 눈에 보이지 않는 버그를 로그에만 의존해 추적하는 과정도 쉽지 않았습니다. 중간 평가 때 교수님께서 제안해주신 개입 기능을 구현하면서는, 프론트엔드와 백엔드가 면접 상태를 각자 관리하고 있어 설계 단계의 리팩토링부터 막혔고, 확장성을 고려하지 않은 코드가 이후에 얼마나 큰 비용이 되는지 깨달았습니다. 시간이 더 있었다면 더 많은 실험 참가자를 모아 피드백으로 품질을 높이고 싶다는 아쉬움도 남습니다. 처음 경험한 장기 협업을 통해, 결합도가 높은 기능일수록 코드를 작성하기 전에 팀원과 충분히 소통하는 것이 중요하다는 것을 배웠습니다.
 
**강동요**
 
STT와 TTS를 구현하며 최신 오픈소스 모델을 직접 다뤄보고, 과거 모델보다 크게 향상된 인식·합성 정확도에 감명받았습니다. Whisper와 Fish Speech를 서로 다른 하드웨어 환경에서 연동하며 CUDA와 라이브러리 버전 충돌을 겪었지만, Docker 컨테이너를 도입해 설정 변경만으로 해결할 수 있었습니다. 다만 지연 시간을 줄이기 위해 STT와 TTS를 모두 로컬에서 구동한 구조에는 아쉬움이 남습니다. 상용 수준의 지연 단축이 우선이었다면, 교수님의 최종 피드백처럼 음성을 텍스트로 바꿨다가 다시 합성하는 대신 오디오 스트림을 직접 처리하는 End-to-End 음성 모델 API를 활용하는 편이 더 올바른 접근이었을 것입니다. 협업 면에서는 다른 팀원의 선행 구현을 기다려야 하는 병목을 테스트용 더미 엔드포인트로 풀어 독립적으로 개발을 이어갈 수 있었고, 결합도가 높은 기능일수록 코드 작성 전에 API 인터페이스 규격을 명확히 정하는 것이 소통 비용과 구현 복잡도를 줄인다는 점을 깊이 체감했습니다.
 
**강저릭 어요솝드**
 
LLM 파트를 맡아 GPT-4o-mini로 답변 채점, 질문 생성, 페르소나 판단을 한 번에 처리하는 프롬프트를 설계하며 구조화된 출력 능력에 감명받았습니다. 부분 전사만으로 주제 이탈을 실시간 판단하다 보니 초반에는 오탐이 잦았고, 프롬프트와 판단 기준을 반복해서 다듬어 해결했습니다. 돌아보면 채점·질문 생성·개입 판단을 매 턴 하나의 LLM 호출에 맡긴 구조가 아쉽습니다. 개입 여부처럼 가벼운 판단은 경량 모델이나 규칙 기반 필터로 먼저 거르고 LLM은 복잡한 판단에만 쓰는 편이 지연 시간과 오탐률 모두에 유리했을 것입니다. STT를 로컬에서 직접 실행할 환경이 없어 연동 테스트는 팀원에게 맡겼지만, STT 출력 형식을 미리 공유받아 채점 로직을 독립적으로 개발했고, 직접 실행할 수 없는 부분이 있어도 인터페이스만 명확하면 협업이 어렵지 않다는 것을 배웠습니다.
